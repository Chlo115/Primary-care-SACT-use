...
To quantify on how many occasions in the last 4 years (2019-2022) SACT has been prescribed and dispensed in Primary Care in England.
...
On `r sum(small_quantity_tablet$Items)` (`r scales::percent(sum(small_quantity_tablet$Items)/total_items)`) occassions, fourteen doses of SACT or less were dispensed. Of these, `r sum(single_tablet$Items)` (`r scales::percent(sum(single_tablet$Items)/total_items)`) were single doses.
...
We observed an unexpected proportion of the dispensed items were in quantities of fourteen or less; suggesting possible emergency supplies. National guidance on the appropriateness of these kinds of supply would be beneficial.
